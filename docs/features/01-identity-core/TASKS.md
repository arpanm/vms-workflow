...
- [x] F01-T02 Add canonical engagement fields/config versions, projects, teams, contacts/contact groups and engagement role assignments without dropping legacy columns.
- [x] F01-T03 Add approval policy versions/stages, requests/actions and effective-dated delegation foundations.
- [x] F01-T04 Add engagement months, guarded state transitions and append-only transition history.
...
- [x] F01-T06 Add PostgreSQL roles/least-privilege access as required; prohibit anonymous legacy data access in the replacement path and do not rely on client/UI controls for authorization.
- [x] F01-T07 Add constraints for organization separation, unique scope keys, first-of-month dates and state/version integrity.
...
- [ ] F01-T09 Add OIDC/JWT login/logout/session integration and protected Spring API endpoints; document issuer, audience, claims and key-rotation configuration. JWT resource-server enforcement and session claims are complete locally; approved provider/BFF login, logout, provisioning and key-rotation acceptance remain external.
- [x] F01-T10 Make demo role switching available only in non-production demo mode with a visible banner.
- [x] F01-T11 Add current organization/engagement/month scope and permission-aware navigation.
...
- [x] F01-T14 Add typed domain errors for unauthorized, conflict, invalid transition and stale version.
...
- [x] F01-T16 Generate/validate OpenAPI from executable controllers; generate frontend client types only from that contract if needed.
...
The local product exit gate is complete. The only unchecked item is the
controlled-environment identity acceptance portion of F01-T09; local signed
JWT/JWKS tests cannot stand in for an approved provider tenant, BFF cookie
session, invitation/provisioning, MFA and key-rotation exercise.
