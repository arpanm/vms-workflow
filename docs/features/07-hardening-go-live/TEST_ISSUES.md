...
| F07-TEST-004 | P1 | CONTRACT/REPRO CHECK PASS; FINAL PROVENANCE PENDING | T070 now requires a dedicated exact-clean-commit manifest. Focused isolated rebuild comparison matched both frontend and backend release digests; execute the manifest again after the worktree is clean and bind all final outputs. |
...
| F07-TEST-011 | P1 | FIXED / VERIFIED LOCALLY; COMMIT BINDING PENDING | The first supply-chain run failed as designed. The exact complete post-fix rerun passes all scanner reports, both release artifacts and the PostgreSQL image with zero findings; Maven R4 passes 290/290 on PostgreSQL 18.4. T070 consumes the per-artifact SBOMs only from a passing supply result and independently rebuilds both artifacts. Repeat from the clean remediation commit for provenance binding. |
