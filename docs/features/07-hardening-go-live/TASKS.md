...
Checkboxes record implemented and directly tested local controls, not a release
verdict. The current release still requires the separate current-commit closure
listed below. Historical evidence is additive and does not erase earlier
failed runs:
...
- the production schema history used by the current product is V1–V42. The
  recorded F07 R3/R4 and supply-chain bundles predate V39/V40 and therefore
  remain historical evidence rather than current-release evidence. V1000+
  migrations are test
  fixtures only;
...
Current local closure is still required for T057, T066–T067, T070, T079,
T081–T082 and the local side of T085: a real 24-hour local soak report, a
controlled recovery-boundary drill, current V1–V42 artifacts/provenance,
contextual/user guidance verification, the complete current regression, and an
exact-current-release independent review. Every `EXTERNAL — ACTION_REQUIRED`
task and the overall production decision remain open/`NO-GO`.

## A. Release evidence, traceability and configuration

- [x] **F07-T001 — LOCAL:** Create a machine-readable release-evidence
...
- [x] **F07-T002 — LOCAL:** Add a CI/local release-gate command that rejects
...
- [x] **F07-T004 — LOCAL:** Implement the production-readiness configuration
...
- [x] **F07-T005 — LOCAL:** Fail closed at startup or workflow entry when a
...
- [x] **F07-T006 — LOCAL:** Preserve Java/PostgreSQL architecture precedence in
...
- [x] **F07-T009 — LOCAL:** Complete the HTTP/controller/service/object-scope
...
- [x] **F07-T010 — LOCAL:** Add adversarial MockMvc/Testcontainers tests for
  ...
  mismatch.
- [x] **F07-T011 — LOCAL:** Define separate PostgreSQL migration-owner,
...
- [x] **F07-T012 — LOCAL:** Add PostgreSQL integration tests proving the
...
- [x] **F07-T013 — LOCAL:** Review every database function/view for fixed
...
- [x] **F07-T014 — LOCAL:** Enforce active identity/membership/authority checks
...
- [x] **F07-T015 — LOCAL:** Add service-account identities with non-interactive,
  minimal scopes and tests proving human-only operations are unavailable.
- [x] **F07-T016 — LOCAL:** Protect Swagger/OpenAPI and Actuator endpoints by
...
- [x] **F07-T018 — LOCAL:** Add a security-header policy for CSP,
...
- [x] **F07-T019 — LOCAL:** Implement an explicit production CORS allowlist;
  reject wildcard origins with credentials and untrusted preflight requests.
- [x] **F07-T020 — LOCAL:** Document the JWT bearer/API CSRF boundary; require
...
- [x] **F07-T021 — LOCAL:** Centralize validation, output encoding and safe
...
- [x] **F07-T022 — LOCAL:** Centralize outbound HTTP clients with HTTPS,
...
- [x] **F07-T023 — LOCAL:** Apply actor/scope-aware rate limits to
...
- [x] **F07-T024 — LOCAL:** Standardize problem responses with a correlation ID
  and safe code/detail; prevent stack traces, SQL, tokens, PII and provider
  payloads from reaching clients or ordinary logs.
- [x] **F07-T025 — LOCAL:** Add replay/idempotency abuse tests for punches,
  approvals, confirmations, webhooks, imports, exports and job commands under
  concurrency.
- [x] **F07-T026 — LOCAL:** Verify geolocation, biometric, selfie, keystroke and
...
- [x] **F07-T027 — LOCAL:** Inventory all environment variables and secret
...
- [x] **F07-T028 — LOCAL:** Add repository secret scanning, SAST, Java/Node
  dependency vulnerability checks, license inventory and container/SBOM
  generation to the release gate.
- [x] **F07-T029 — LOCAL:** Pin/review build inputs, produce checksums/SBOM and
...
- [x] **F07-T031 — LOCAL:** Sanitize or remove committed local `.env` material,
...
- [x] **F07-T033 — LOCAL:** Apply shared upload limits, content sniffing,
  ...
  artifacts.
- [x] **F07-T034 — LOCAL:** Ensure failed/pending/quarantined artifacts cannot
...
- [x] **F07-T035 — LOCAL:** Add malicious-file corpus tests (EICAR test string,
  MIME mismatch, macro/active HTML, archive bomb metadata, path traversal,
  oversized body and duplicate hash).
- [x] **F07-T036 — LOCAL:** Create and enforce a data-classification inventory
...
- [x] **F07-T037 — LOCAL:** Implement privacy-minimized exports/packages,
...
- [x] **F07-T040 — LOCAL:** Complete immutable audit coverage for identity,
  ...
  security actions.
- [x] **F07-T041 — LOCAL:** Propagate trusted correlation and causation IDs
...
- [x] **F07-T042 — LOCAL:** Add append-only PostgreSQL grants/triggers and
  periodic hash/count integrity verification for audit/security/evidence
  records; corrections must append compensating facts.
- [x] **F07-T043 — LOCAL:** Implement configurable retention schedules by
...
- [x] **F07-T044 — LOCAL:** Implement legal-hold placement/release with scoped
...
- [x] **F07-T046 — LOCAL:** Add Micrometer metrics and structured logs for API
...
- [x] **F07-T047 — LOCAL:** Define SLI/SLO dashboards, alert severity, owner,
  deduplication key, threshold, runbook link and correlation drill-down; avoid
  high-cardinality PII labels.
- [x] **F07-T048 — LOCAL:** Add health/readiness checks that distinguish
...
- [x] **F07-T049 — LOCAL:** Prove bounded retry/backoff, idempotent
...
- [x] **F07-T050 — LOCAL:** Add failure injection for database interruption,
...
- [x] **F07-T051 — LOCAL:** Preserve last-known provider state with explicit
...
- [x] **F07-T064 — LOCAL:** Provide scripts/runbooks for encrypted PostgreSQL
...
- [x] **F07-T065 — LOCAL:** Automate restore into an isolated environment,
...
- [x] **F07-T072 — LOCAL:** Add pre-deploy Flyway validate/migrate rehearsal,
...
- [x] **F07-T073 — LOCAL:** Define canary cohorts, success/error/latency/data
...
- [x] **F07-T075 — LOCAL:** Preserve all new events/data during rollback,
...
- [x] **F07-T079 — LOCAL:** Add role-specific user/support guides and in-app
