...
| F07 local-system E2E | Ordered browser/API workflows through Vite, Spring Security, Flyway and PostgreSQL | F07 system runner | Current V1–V43 **7/7**: E2E-01/02/03/04/05/07/10. V42 is repeated-reopen; V43 is durable migration. |
...
| F07 backend/operations | Java/PostgreSQL hardening plus release, supply-chain, migration, rollout and recovery harness | `mvn -B -f backend/pom.xml verify`, `npm run f07:self-test`, `npm run f07:ops:check` | Self-test 9/9 (45.037 s), operations 15 runbooks/6 alerts, 43 migration schemas, rollout schema and SDLC eight-feature check pass. `f07:release:schema` wrapper is unverified due to sandbox bind `EPERM` and a 467 s silent escalated retry; underlying gates pass. |
