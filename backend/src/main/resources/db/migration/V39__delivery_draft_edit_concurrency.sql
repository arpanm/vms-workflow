-- Permit only a version-bumping edit of an existing draft. Terminal and
-- submitted evidence rules remain identical to V10.
    ...
    IF OLD.state = 'DRAFT' AND NEW.state = 'DRAFT' THEN
        IF NEW.id IS DISTINCT FROM OLD.id
           ...
           OR NEW.created_by_subject IS DISTINCT FROM OLD.created_by_subject THEN
            RAISE EXCEPTION 'Draft edit must preserve identity and advance its lock version'
                USING ERRCODE = '55000';
        END IF;
        RETURN NEW;
    END IF;
    ...
    IF OLD.state = 'PENDING_APPROVAL'
       AND NEW.state IN ('FROZEN', 'REJECTED') THEN
        ...
        IF required_approval_count IS NULL
           ...
           OR EXISTS (
               SELECT 1 FROM delivery_plan_approvals approval
               WHERE approval.plan_version_id = NEW.id
                 AND approval.decision = 'REJECT'
           )
           OR NOT EXISTS (
               SELECT 1 FROM delivery_plan_baselines baseline
           ...
           OR NOT EXISTS (
               ...
               JOIN delivery_plan_baselines baseline ON baseline.id = outbox.baseline_id
